...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def create_source_playlist(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None, is_radio, is_file = False):
        if is_file:
            info = {"title": "Uma Merda Qualquer ®",
                    "url": search, 
                    "uploader": ctx.author}
            ...
            if 'entries' not in data:
                ...
                process_info = set()
                for entry in data['entries']:
                    process_info.add(entry.get('url'))
